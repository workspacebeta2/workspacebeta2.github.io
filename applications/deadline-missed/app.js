		...
		change: function(e){
			...
			$("#ws-ui-113762-1").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbzW7IQKU-4LKGg5mfgPfhODWF5dDslSZTxZ66GR9GkD-DdjYC0/exec?type=deadline-missed&rl="+window.authUser[0].Role+"&comp=cd&date="+udate1
						}
					},
			...
			$("#ws-ui-113762-2").kendoGrid({
				dataSource: {
					...
					transport: {
						read: {
							url: "https://script.google.com/macros/s/AKfycbzW7IQKU-4LKGg5mfgPfhODWF5dDslSZTxZ66GR9GkD-DdjYC0/exec?type=deadline-missed&rl="+window.authUser[0].Role+"&comp=ad&date="+udate2
						}
					},
					/*schema: { model: { id: "sl", fields: {
						UserName: { type: "string" }, 
